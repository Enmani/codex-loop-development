...
一套面向 Codex 桌面端的循环开发编排协议：让一个“包工头”任务按计划调度多个开发任务，独立完成审核、返工、受约束的计划修订、下一阶段派发和最终收尾；另设一个 Luna xhigh 监管任务，只在消息链或状态转换断开时兜底。总包任务损坏时，系统可以从持久状态自动创建继任总包。
...
- 包工头是唯一审核、返工和派工决策者。
- worker 发现遗漏前置、错误依赖或拆阶段需求时只报告 `PLAN_GAP_FOUND`；只有当前包工头可以修改正式计划。
- 包工头可自主修补不改变用户目标和验收标准的技术闭环，并为每次变更递增 `planRevision`、更新指纹、记录原因、重算 DAG 和广播 `PLAN_REVISED`。
- 若计划变更会影响正在施工的任务，总包先完成 `PLAN_PAUSE`/`PLAN_PAUSE_ACK` 安全边界握手，再编辑计划；旧授权不会和新 DAG 并行。
- 改产品目标/范围、降低验收、移除用户门、引入外部成本/凭据、不可逆操作或改变活动 worker 权限，必须先获得用户明确批准。
- 等待这类批准时，运行进入 `awaiting_user` 且监管保持 `NO_OP`；必须明确批准对应 proposal/gate，模糊的“继续”不会被当成授权。
...
- worker 发现计划缺口时不得自行修改主计划；总包可自主修补技术闭环，涉及目标、范围、验收、用户门、外部成本/凭据或不可逆权限时必须先询问我。
