...
你只需要先写好计划，然后在一个 Codex 任务中说“开始循环开发”。系统会自动建立包工头、开发任务和独立监管任务，完成派工、审核、返工、受约束的计划修订、下一阶段派发与最终收尾。总包任务损坏时，还可以从持久状态自动创建继任总包。
...
- 运行阶段要求的测试。
- 整理证据并主动发回包工头。
- 发现计划遗漏前置或依赖错误时提交缺口报告，不自行修改正式计划。
- 收到返工要求后继续在原任务中修复。
...
| `planRevision` | 当前正式计划版本，从 1 开始；每次应用计划变更只能加 1。 |
| `planFingerprint` | 当前计划文件的内容指纹，用来防止总包或继任总包误读旧计划。 |
...
- 若主计划与 Checklist 对用户目标、范围或验收相互冲突，必须停止并让用户决定；若只是实现过程中暴露的遗漏前置或错误依赖，则可由包工头按版本化规则修补。
...
- worker 发现计划缺口时不得自行修改主计划；包工头可自主修补不改变用户目标和验收的技术闭环，越过目标、范围、验收、用户门、外部成本/凭据或不可逆权限时必须先询问我。
...
| `PLAN_GAP_FOUND` | worker 发现当前计划无法安全完成，提交证据和最小修补建议 |
| `PLAN_PAUSE` / `PLAN_PAUSE_ACK` | 修改计划前，让受影响的活动 worker 在安全边界停下并确认 |
| `PLAN_REVISED` | 包工头已应用新计划版本，相关 worker 需要重新读取并核对授权 |
...
最常见的例子是：G08 的验收要求需要一份“当前生产证据”，但现有阶段没有任何任务负责生成它。worker 不应该偷偷放宽 G08 的验收，也不应该自己在主计划里加活；它会把事实写入 Outbox，并发送 `PLAN_GAP_FOUND`。

包工头随后分两类处理：

- 可以自动修订：补遗漏前置、修正依赖、拆分既有阶段、加强为达到原目标所需的验证。例如新增 `G07C` 负责供应证据，并让 G08 依赖 G07C。
- 必须问用户：改变产品目标或范围、降低验收标准、移除或越过用户门、引入外部服务/成本/凭据、执行不可逆操作，或改变正在工作的 worker 权限。

自动修订时，若有受影响的活动 worker，包工头会先写入暂停请求并发送 `PLAN_PAUSE`；worker 在安全边界停下、回复 `PLAN_PAUSE_ACK` 后，包工头才释放其锁并编辑计划。主动提交 `PLAN_GAP_FOUND` 且声明已到安全边界的 worker 已完成这次确认。随后包工头修改计划和 Checklist，验证新的依赖图，把 `planRevision` 加 1、更新 `planFingerprint`、记录变更原因，并发送 `PLAN_REVISED`。不受影响的并行 worker 可以继续；已暂停的 worker 必须等待刷新后的派工或返工消息，单独收到新版本通知不等于恢复授权。监管只检查这些转换有没有完成，不替包工头决定计划怎么改。

必须问用户时，受影响阶段保持 blocked，运行写入 `awaiting_user`，`expectedNext` 设为空，监管返回 `NO_OP`。包工头会给这次变更一个明确的 proposal/gate ID；你需要针对它明确批准。模糊的“继续”不会授权降低验收、花费、凭据或外部副作用。涉及外部服务时，服务名称、费用上限、凭据和数据处理、允许的副作用都要分别说明；批准改计划不等于凭据已经可用。
...
会，但只限技术闭环。包工头可以补前置、修依赖、拆阶段或加强验证，不能改变你的产品目标、降低验收或扩大外部授权。后一类会停在 `awaiting_user`，等你明确批准后才写入新计划版本。
...
- 让 worker 直接修改主计划，或让包工头借“技术修补”降低验收和扩大产品范围。
- worker 完成后仍让包工头持续轮询。
- 报告丢失时反复复制整份报告。
...
8. 若施工暴露计划缺口，让 worker 报告、包工头修订并广播新版本；涉及用户边界时由你批准。
9. 遇到用户门时，带上 `runId` 和 `gateId` 输入 `go`、`hold` 或 `redirect`。
10. 若总包发送入口明确失败，让系统先自动 archive → unarchive 并重发；重发仍失败后再触发接管，不要手工同时创建第二个总包。
11. 应用重启后带上 `runId` 输入“继续循环开发”，先从 durable state 与 Outbox 恢复。
12. 全部阶段正式通过后，确认相关自动任务自动变为 `INACTIVE`。
...
- worker 发现计划缺口时只写入 Outbox 并发送 PLAN_GAP_FOUND，不得自行修改正式计划或扩大授权。
- 本包工头可以自主补充遗漏前置、纠正依赖、拆分阶段或加强验证，但不得改变我的产品目标/范围、降低验收、移除用户门、引入外部成本/凭据、授权不可逆操作或改变活动 worker 权限；后一类必须先进入 awaiting_user 等我批准。
- 每次应用计划变更必须让 planRevision 恰好加 1，更新 planFingerprint 和 lastPlanChange，重算 DAG，并向监管及受影响 worker 发送 PLAN_REVISED。
