...
你只需要先写好计划，然后在一个 Codex 任务中说“开始循环开发”。系统会自动建立包工头、开发任务和独立监管任务，完成派工、审核、返工、下一阶段派发与最终收尾。
...
监管任务不能审核代码、不能决定通过、不能修改仓库，也不能代替包工头派工。
...
如果 Codex 桌面端或本地 app server 曾经关闭，定时任务在关闭期间可能不会运行。重新打开后，监管的下一次心跳会读取持久状态并补齐缺失转换。
...
可以在启动 Prompt 中额外写明模型策略，但默认 worker 使用项目或任务的现有模型。独立监管固定推荐 Luna xhigh。
...
9. 应用重启后带上 `runId` 输入“继续循环开发”，先从 durable state 恢复。
10. 全部阶段正式通过后，确认相关自动任务自动变为 `INACTIVE`。
